...
raw$Cancer_weighted_5yr_sum <- apply(raw, MARGIN = 1, FUN = function(x) {
  lbs<-as.numeric(x[c("PoundsReleased_2016", "PoundsReleased_2017", "PoundsReleased_2018", "PoundsReleased_2019", "PoundsReleased_2020")])
...
#DEV chemicals only
Dev <- c("1,2-Dichloroethane", "1,2-Dibromoethane (Ethylene dibromide)", "1,2-Dichloropropane", "1,3-Butadiene", "1,4-Dioxane", "1-Bromopropane", "Dibutyl phthalate", "Di(2-ethylhexyl) phthalate", "N-Methyl-2-pyrrolidone", "Tetrachloroethylene", "Trichloroethylene", "Hexabromocyclododecane")

raw$Dev_PoundsReleased_5yr_min <- apply(raw, 1, function(row) {
  ifelse(row["Chemical"] %in% Dev, min(as.numeric(row[c("PoundsReleased_2016", "PoundsReleased_2017", "PoundsReleased_2018", "PoundsReleased_2019", "PoundsReleased_2020")]), na.rm = TRUE), 0)
})
raw$Dev_PoundsReleased_5yr_max <- apply(raw, 1, function(row) {
  ifelse(row["Chemical"] %in% Dev, max(as.numeric(row[c("PoundsReleased_2016", "PoundsReleased_2017", "PoundsReleased_2018", "PoundsReleased_2019", "PoundsReleased_2020")]), na.rm = TRUE), 0)
})
...
Asthma <- c("1,2-Dibromoethane (Ethylene dibromide)", "Formaldehyde", "Phthalic anhydride")
...
#filtering/QC
test2 = final %>% 
  filter(FacilityID == "59414MNTNR19001") %>% select(everything())
